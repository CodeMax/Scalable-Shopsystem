...
export class RegisterComponent {
    ...
    onRegisterSubmit(username: string, firstname: string, lastname: string,
      ...
      this._newUser = new User(0, username, firstname, lastname, password, address, postcode, city, country, 0);
    ...
    saveUserDetail(token: Token) {
      ...
      new BackendcallService(this._http, 'token', JSON.stringify(token), 'http://192.168.99.100:8087/user/' + this._newUser.id)
              .updateUserData(this._newUser.id, this._newUser.firstname, this._newUser.lastname, this._newUser.address,
